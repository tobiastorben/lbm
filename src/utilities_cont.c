...
void outlet(double* ux, double* uy, double* rho, double* fIn, int nx, int ny) {
	double sum1, sum2;
	for (int j = 1; j < (ny-1);j++) {
		sum1 = fIn[(nx-1)+j] + fIn[2*nx*ny+(nx-1)+j] + fIn[4*nx*ny+(nx-1)+j];
		sum2 = fIn[1*nx*ny+(nx-1)+j] + fIn[5*nx*ny+(nx-1)+j] + fIn[8*nx*ny+(nx-1)+j];
		ux[(nx-1) +j] = -1.0 +sum1 + 2.0*sum2;//UNCERTAIN!
		uy[(nx-1) +j] = 0;
		fIn[3*nx*ny+(nx-1)+j] = fIn[1*nx*ny+(nx-1)+j] - (2.0/3)*ux[(nx-1) +j];
		fIn[7*nx*ny+(nx-1)+j] = fIn[5*nx*ny+(nx-1)+j] + 0.5*(fIn[2*nx*ny+(nx-1)+j]-fIn[4*nx*ny+(nx-1)+j])
					   -(1.0/6)*ux[(nx-1) +j];
		fIn[6*nx*ny+(nx-1)+j] = fIn[8*nx*ny+(nx-1)+j] + 0.5*(fIn[4*nx*ny+(nx-1)+j]-fIn[2*nx*ny+(nx-1)+j])
					  -(1.0/6)*ux[(nx-1) +j];
		rho[(nx-1) +j] = 1.0;
	}
}
...
void stream(double* fIn, double* fOut,int* ex,int* ey, int nx, int ny, int nf) {
	...
	for (k = 0; k < nf; k++) {
			if (ex[k]==1){
				if (ey[k]==-1) fIn[k*nxny+ ny-1] = fOut[k*nxny+(nx-1)*ny+0];
